    ...
    try {
      ...
      if (editTarget) {
        ...
        createAuditLog({
          ...
          description: `Renamed storage option "${editTarget.name}" to "${formName.trim()}"`,
          oldValue: JSON.stringify({ name: editTarget.name }), newValue: JSON.stringify({ name: formName.trim() }),
        }).catch(() => {})
      } else {
        const { data: created, error } = await supabase.from("storage_options").insert({ tenant_id: tenantId, name: formName.trim(), is_system: false }).select("id").single()
        if (error) throw error
        ...
        createAuditLog({
          ...
          description: `Added storage option "${formName.trim()}"`,
  ...
  async function handleDelete() {
    ...
    try {
      ...
      createAuditLog({
        ...
        description: `Deleted storage option "${deleteTarget.name}"`,
