...
export default function NewSalePage() {
  ...
  const productResults = useMemo((): ProductResult[] => {
    ...
    const uResults: ProductResult[] = (typeFilter === "All" || typeFilter === "UsedPhone") ? usedPhones
      ...
      .map(u => ({ id: u.id, productId: u.id, name: `${u.brand} ${u.model}${u.condition_grade ? ` (Used - ${u.condition_grade})` : " (Used)"}`, type: "UsedPhone" as const, price: u.selling_price ?? 0, costPrice: (u.purchase_price || 0) + (u.refurbishment_cost || 0), stock: 1, imei: u.imei_number, color: u.color, storage: u.storage })) : []
