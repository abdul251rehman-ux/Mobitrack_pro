      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            View financial records and running balance for any customer
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => {
              ...
              const html = `<!DOCTYPE html><html><head><title>Customer Ledger — ${custLabel}</title>
                <style>
                  ...
                  .debit{color:#3b82f6;font-weight:600}
                  .credit{color:#10b981;font-weight:600}
                  .balance-dr{color:#f59e0b;font-weight:700}
                  .balance-cr{color:#10b981;font-weight:700}
                ...
                <table>
                  <thead><tr><th>Date</th>${!selectedCustomerId ? "<th>Customer</th>" : ""}<th>Reference</th><th>Description</th><th class="text-right">Debit</th><th class="text-right">Credit</th><th class="text-right">Balance</th></tr></thead>
                  <tbody>
                    ${filtered.map(e => `<tr>
                      <td>${formatDate(e.date)}</td>
                      ${!selectedCustomerId ? `<td>${e.customerName || "—"}</td>` : ""}
                      ...
                      <td class="text-right ${e.balance > 0 ? "balance-dr" : "balance-cr"}">Rs ${Math.abs(e.balance).toLocaleString()} ${e.balance > 0 ? "Dr" : e.balance < 0 ? "Cr" : ""}</td>
                    </tr>`).join("")}
                    <tr class="total-row">
                      <td colspan="${!selectedCustomerId ? 4 : 3}" class="text-right">Totals</td>
                      <td class="text-right debit">Rs ${totalDebit.toLocaleString()}</td>
                      <td class="text-right credit">Rs ${totalCredit.toLocaleString()}</td>
                      <td class="text-right ${closingBalance > 0 ? "balance-dr" : "balance-cr"}">Rs ${Math.abs(closingBalance).toLocaleString()} ${closingBalance > 0 ? "Dr" : closingBalance < 0 ? "Cr" : ""}</td>
                    </tr>
                  </tbody>
                </table>
          ...
          <button
            onClick={() => {
              ...
              a.href = url; a.download = `customer-ledger-${new Date().toISOString().split("T")[0]}.csv`; a.click()
          ...
          {selectedCustomerId && (
            <div className="mt-4 flex flex-wrap items-center gap-3 pt-4 border-t border-slate-100">
              ...
              <input
                ...
                placeholder="0"
              />
              <span className="text-xs text-slate-400">Positive = customer owes, Negative = advance</span>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <Card className="border-l-4 border-l-blue-500">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Debit</p>
                <TrendingUp className="w-4 h-4 text-blue-400" />
              </div>
              <p className="text-xl font-bold text-slate-900 mt-1">{formatCurrency(totalDebit)}</p>
              <p className="text-xs text-slate-400 mt-0.5">Amount charged to customer</p>
            </CardContent>
          </Card>
          <Card className="border-l-4 border-l-emerald-500">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between">
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Credit</p>
                <TrendingDown className="w-4 h-4 text-emerald-400" />
              </div>
              <p className="text-xl font-bold text-slate-900 mt-1">{formatCurrency(totalCredit)}</p>
              <p className="text-xs text-slate-400 mt-0.5">Payments received</p>
            </CardContent>
          </Card>
          <Card
      ...
      {filtered.length === 0 ? (
        <Card>
          <CardContent className="py-16 text-center">
            <p className="text-slate-400 font-medium">No transactions found for this customer in the selected period.</p>
          </CardContent>
        </Card>
      ) : (
        <Card>
            ...
            <div className="md:hidden divide-y divide-slate-100">
                  ...
                  <div className="flex-1 px-3 py-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="text-right flex-shrink-0">
                        {entry.debit > 0 && (
                          <p className="text-sm font-semibold text-blue-600">Dr {formatCurrency(entry.debit)}</p>
                        )}
                        {entry.credit > 0 && (
                          <p className="text-sm font-semibold text-emerald-600">Cr {formatCurrency(entry.credit)}</p>
                        )}
                        <p
              ...
              <div className="px-4 py-3 bg-slate-50 border-t-2 border-slate-200">
                ...
                <div className="flex justify-between text-sm mt-1 pt-1 border-t border-slate-200">
                  ...
                  <span
                    className={`font-bold ${
                      closingBalance > 0 ? "text-amber-600" : closingBalance < 0 ? "text-emerald-600" : "text-slate-400"
                    }`}
                  >
                    {formatCurrency(Math.abs(closingBalance))}
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="border-t-2 border-slate-300 bg-slate-50 font-semibold">
                    ...
                    <td className="px-4 py-3 text-right text-blue-700 whitespace-nowrap">
