...
export function generateReportPDF(opts: ReportOptions): void {
  ...
  // Footer — always on last page, pinned to page bottom
  const pageH  = orientation === "landscape" ? 210 : 297
  const footH  = 13
  const footY  = pageH - footH - 2.5
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text("Generated by", M, footY + 8)
    doc.setFont("helvetica", "bold")
    doc.setTextColor(...BLUE)
    doc.text(shopName, M + doc.getTextWidth("Generated by ") + 1, footY + 8)

    doc.setFont("helvetica", "normal")
    doc.setTextColor(...SLATE4)
    doc.text(
      `${rows.length} record${rows.length !== 1 ? "s" : ""}  -  Printed ${new Intl.DateTimeFormat("en-PK", { timeZone: "Asia/Karachi", day: "2-digit", month: "short", year: "numeric" }).format(new Date())}  -  Page ${p} of ${totalPages}`,
      W / 2, footY + 8, { align: "center" }
    )
    doc.text(title, W - M, footY + 8, { align: "right" })

    gradientRule(doc, 0, footY + footH, W, 2)
  }
