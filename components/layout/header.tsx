      ...
      <header className="sticky top-0 z-30 h-16 bg-white border-b border-slate-200 flex items-center px-3 md:px-6 gap-2 md:gap-4 shrink-0 shadow-sm">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="relative w-9 h-9 rounded-xl border border-slate-200 flex items-center justify-center hover:bg-slate-50 hover:border-slate-300 transition-all">
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] rounded-full bg-rose-500 text-white text-[10px] flex items-center justify-center font-bold px-1 border-2 border-white">
                  {unreadCount}
                </span>
              )}
            </button>
              ...
              {notifications.map(n => (
                <DropdownMenuItem key={n.id} className="flex flex-col items-start gap-1 py-3 px-3 cursor-default">
                  <div className="flex items-center gap-2 w-full">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${n.read ? "bg-transparent" : "bg-indigo-500"}`} />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-52">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => router.push("/settings")}><User className="w-4 h-4" /> Profile</DropdownMenuItem>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem className="gap-2 text-rose-600 cursor-pointer" onClick={async () => { await logout(); router.push("/auth/login") }}><LogOut className="w-4 h-4" /> Logout</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </header>
